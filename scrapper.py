...
sql = "CREATE DATABASE facebook_db"  # Replace "mydatabase" with your desired database name
...
sql = "CREATE TABLE IF NOT EXISTS  fb_page \
    ...
    post_date DATE NOT NULL)"  # Replace with your desired table and column structure
mycursor.execute(sql)

for item_ in data_dict:
    page_name = item_['name']
    shares_count = item_['shares']
    comments_count = item_['comments']
    post_content = item_['content']
    post_date = item_['posted_on']
